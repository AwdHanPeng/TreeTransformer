...
class Node:
    def __init__(self, idx, node_type, data, children, child_rel=[], father_rel=[]):
        """
        D-ary tree.
        """
    ...
    def build_tree(ast, i=0, child_rel=[], father_rel=[]):
        ...
        if children is None:
            return Node(i, node_type, data, [], child_rel, father_rel)
        else:
            total = len(children)
            children = [child for child in children if child < len(ast)]
            children = [Node.build_tree(ast, j, child_rel + [child_i], father_rel + [total]) for child_i, j in
                        enumerate(children)]
            return Node(i, node_type, data, children, child_rel, father_rel)
