...
do
    ...
    function create_td_tree_relative_encodings () {
            output_dir=$1
            if [ $root_paths = "True" ]; then
                echo "---CREATE TREE RELATIVE ENCODINGS----"
                if [ ! -f $out ] || [ $from_scratch = "True" ]; then
                    echo "Creating root paths..."
                    $PYTHON ./src/scripts/generate_root_paths.py --ast_fp $path/$train.json -o $output_dir/train/td_paths.txt --n_ctx $n_context --td
                    $PYTHON ./src/scripts/generate_root_paths.py --ast_fp $path/$eval.json  -o $output_dir/eval/td_paths.txt  --n_ctx $n_context --td
                    $PYTHON ./src/scripts/generate_root_paths.py --ast_fp $path/$test.json  -o $output_dir/test/td_paths.txt  --n_ctx $n_context --td
                    echo "Created eval root paths at $output_dir"
                else
                    echo "TD Root paths are already created at $out Skip."
                fi
            fi
        }
    function create_lr_tree_relative_encodings () {
            output_dir=$1
            if [ $root_paths = "True" ]; then
                echo "---CREATE TREE RELATIVE ENCODINGS----"
                if [ ! -f $out ] || [ $from_scratch = "True" ]; then
                    echo "Creating root paths..."
                    $PYTHON ./src/scripts/generate_root_paths.py --ast_fp $path/$train.json -o $output_dir/train/lr.txt --n_ctx $n_context --local_relation
                    $PYTHON ./src/scripts/generate_root_paths.py --ast_fp $path/$eval.json  -o $output_dir/eval/lr.txt  --n_ctx $n_context --local_relation
                    $PYTHON ./src/scripts/generate_root_paths.py --ast_fp $path/$test.json  -o $output_dir/test/lr.txt  --n_ctx $n_context --local_relation
                    echo "Created eval root paths at $output_dir"
                else
                    echo "Root paths are already created at $out Skip."
                fi
...
##            create_td_tree_relative_encodings $output_dir $mode
##            create_lr_tree_relative_encodings $output_dir $mode
#        fi
    }
