...
from src.utils.metric_utils import mrr, acc
...
class LightningModel(pl.LightningModule):
    ...
    def __build_model(self, args):
        ...
        self.types_criterion = MaskedLoss(pad_idx=self.vocab.types_vocab.pad_idx,
                                          oov_idx=self.vocab.types_vocab.unk_idx,
                                          empty_idx=self.vocab.types_vocab.empty_idx) if not self.args.only_values else None
        self.values_criterion = MaskedLoss(pad_idx=self.vocab.values_vocab.pad_idx,
                                           oov_idx=self.vocab.values_vocab.unk_idx,
                                           empty_idx=self.vocab.values_vocab.empty_idx)
    ...
    def train_dataloader(self):
        train_dataloader = DataLoader(self.train_dataset, shuffle=True, batch_size=self.args.batch_size,
                                      num_workers=self.args.num_workers, collate_fn=self.collate_fn, drop_last=True,
                                      pin_memory=True)
    ...
    def val_dataloader(self):
        ...
        eval_dataloader = DataLoader(self.eval_dataset, batch_size=self.args.batch_size,
                                     num_workers=self.args.num_workers, collate_fn=self.collate_fn, drop_last=False,
    ...
    def _step(self, batch, batch_nb, key="train"):
        # main routine
        types, values = self(batch["input_seq"], rel=batch["rel_mask"], positions=batch["positions"], tds=batch["tds"],
                             lr=batch["lr"])
        ...
        if key != "train":
            with torch.no_grad():
                if not self.args.only_values:
                    logs["mrr_types"], logs["mrr_types_n"], logs['type_acc'], logs['type_acc_n'] = mrr(types,
                                                                                                       types_true,
                                                                                                       batch[
                                                                                                           "extended"],
                                                                                                       self.vocab.types_vocab)
                logs["mrr_values"], logs["mrr_values_n"], logs['values_acc'], logs['values_acc_n'] = mrr(values,
                                                                                                         values_true,
                                                                                                         batch[
                                                                                                             "extended"],
                                                                                                         self.vocab.values_vocab)

                logs["acc"], logs["acc_n"] = acc(types, types_true, values, values_true, batch["extended"],
                                                 self.vocab.types_vocab, self.vocab.values_vocab)
    ...
    def add_model_specific_args(parent_parser):
        ...
        # HERE
        parser.add_argument("--use_td_tree_pos_enc", action='store_true', help="use advance tree positional encodings")
        parser.add_argument('--td_max_ary', type=int, default=16,
                            help='maximum width of root path')
        parser.add_argument('--td_max_depth', type=int, default=16,
                            help='maximum depth of root path')
        parser.add_argument('--tree_TD', action='store_true',
                            help='maximum depth of root path')
        parser.add_argument('--action_size', type=int, default=32,
                            help='maximum depth of root path')
        parser.add_argument('--local_relation', action='store_true',
                            help='maximum depth of root path')
        parser.add_argument('--sqrt', type=int, default=-1)
        parser.add_argument("--only_local_relation", action='store_true',
                            help="use advance tree positional encodings")
        parser.add_argument("--td_cat", action='store_true', help="use advance tree positional encodings")
        parser.add_argument("--td_add", action='store_true', help="use advance tree positional encodings")

        parser.add_argument("--only_1", action='store_true', help="use advance tree positional encodings")
        parser.add_argument("--only_2", action='store_true', help="use advance tree positional encodings")
