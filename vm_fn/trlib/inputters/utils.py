...
def process_examples(source,
    ...
    if root_path is not None:
        ...
        if len(rps) != len(code_tokens):
            raise ValueError("len(root path) != len(code tokens): %d %d" % \
    ...
    if td_path is not None:
        first_dim, second_dim = td_path.split("\t") # root_1_1/2_3/2/3 \t root_1_1/2_3/2/3
        ones, twos = first_dim.split(), second_dim.split() # [root,1,1/2,3/2/3] [root,1,1/2,3/2/3]
        if len(ones) != len(code_tokens):
            raise ValueError("len(root path) != len(code tokens): %d %d" % \
                             (len(ones), len(code_tokens)))

        tds = [[[int(one_elem), int(two_elem)] for one_elem, two_elem in
                zip(one.split("/"), two.split("/"))] if one != "root" else [] for one, two in zip(ones, twos)]
    ...
    if local_relation is not None:
        ...
        relations = local_relation.split('\t')
        if len(relations) != len(code_tokens):
            raise ValueError("len(relations) != len(code tokens): %d %d" % \
                             (len(relations), len(code_tokens)))

        relations = [json.loads(relation) for relation in relations]
        #[{"0":[0,2,1],"1":[1,2,0]}] #
    else:
        relations = []
