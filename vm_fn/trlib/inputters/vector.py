...
from trlib.utils.tree_utils import generate_positions, get_adj_matrix, generate_TD_position,generate_local_relations
...
def vectorize(ex, model):
    ...
    # here
    if model.args.use_td_tree_pos_enc:
        td_paths = ex["td_paths"]
    #here
    if model.args.local_relation:
    ...
    #here
    vectorized_ex["use_td_tree_pos_enc"] = False
    vectorized_ex["td_paths_rep"] = None
    vectorized_ex["use_local_relation"] = False
    vectorized_ex["relations"] = None
    ...
    # HERE
    if model.args.use_td_tree_pos_enc:
        vectorized_ex["use_td_tree_pos_enc"] = True
        td_matrix = generate_TD_position(td_paths, model.args.td_max_ary, model.args.td_max_depth)
        vectorized_ex["td_paths_rep"] = td_matrix  # (len(root_paths), max_depth, 2)

    #here
    if model.args.local_relation:
        vectorized_ex["use_local_relation"] = True
        relation_matrix = generate_local_relations(relations,model.args.td_max_ary)
        vectorized_ex["relations"] = relation_matrix  # (len(root_paths),len(root_paths),3)
...
def batchify(batch):
    ...
    # HERE:
    use_td_tree_pos_enc = batch[0]["use_td_tree_pos_enc"]
    use_local_relation = batch[0]["use_local_relation"]
    ...
    # HERE
    code_td_paths = [ex["td_paths_rep"] for ex in batch]
    code_local_relations = [ex["relations"] for ex in batch]
    ...
    # HERE
    if use_td_tree_pos_enc:
        depth = batch[0]['td_paths_rep'].shape[1]
    ...
    # HERE
    code_td_paths_rep = torch.zeros(batch_size, max_code_len, depth, 2, dtype=torch.int) \
        if use_td_tree_pos_enc else None
    code_local_relations_rep = torch.zeros(batch_size, max_code_len, max_code_len, 3, dtype=torch.long) \
        if use_local_relation else None
    ...
    for i in range(batch_size):
        ...
        # HERE
        if use_td_tree_pos_enc:
            code_td_paths_rep[i, :code_td_paths[i].shape[0]].copy_(code_td_paths[i])
        if use_local_relation:
            code_local_relations_rep[i, :code_local_relations[i].shape[0],:code_local_relations[i].shape[0]].copy_(code_local_relations[i])
    ...
    return {
        ...
        "code_td_paths_rep": code_td_paths_rep,
        "code_local_relations_rep": code_local_relations_rep,
