...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, args, head_count, model_dim, d_k, d_v, dropout=0.1,
                 max_relative_positions=0, use_neg_dist=True, \
                 use_tree_relative_attn=False, tree_rel_vocab_size=0,
                 coverage=False, use_td_tree_pos_enc=False, use_local_relation=False):
        ...
        self.hard_relation = args.hard_relation

    def forward(self, key, value, query, mask=None, rel_matrix=None, rel_mask=None, input_tokens=None, \
                layer_cache=None,
                attn_type=None, step=None, coverage=None, td_scores=None, LR=None):
        """
        ...
        # query: batch x heads x query_len x dim
        # key: batch x heads x key_len x dim
        ...
        if self.use_td_tree_pos_enc and not self.only_local_relation:
            scores = scores + td_scores

        if self.use_local_relation:
            (LR_Q, LR_K, LR_map) = LR
            # LR_Q:[hnk] LR_K[hnk],LR_map[bij]
            LR_map = LR_map.unsqueeze(1).expand(-1, LR_Q.shape[0], -1, -1)
            LR_1 = torch.einsum('bhik,bhnk->bhin', query,
                                LR_K.unsqueeze(0).expand(query.shape[0], -1, -1,
                                                         -1))  # query already div self.d_k before!
            LR_1 = torch.gather(LR_1, -1, LR_map)  # bhin->bhij

            LR_2 = torch.einsum('bhnk,bhjk->bhnj', LR_Q.unsqueeze(0).expand(key.shape[0], -1, -1, -1),
                                key) / math.sqrt(self.d_k)
            LR_2 = torch.gather(LR_2, -2, LR_map)  # bhnj->bhij
            scores = scores + LR_1 + LR_2

        if self.use_td_tree_pos_enc or self.use_local_relation:
            if self.sqrt == -1:
                raise Exception('Not Set Sqrt!')
            scores = scores / math.sqrt(self.sqrt)
        # if self.use_td_tree_pos_enc and self.use_local_relation:
        #     # TODO we hard set div sqrt 2
        #     scores = scores / math.sqrt(4)
        # elif self.use_local_relation:
        #     scores = scores / math.sqrt(3)
        # elif self.use_td_tree_pos_enc:
        #     scores = scores / math.sqrt(2)

        if self.use_local_relation:
            if self.hard_relation:
                assert self.only_local_relation
                # bhij
                LR_map = LR_map + torch.eye(LR_map.shape[-1], device=LR_map.device, dtype=torch.long).unsqueeze(
                    0).unsqueeze(0)
                scores = scores.masked_fill(LR_map == 0, -1e18)
