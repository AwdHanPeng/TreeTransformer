...
class Transformer(nn.Module):
    ...
    def get_logits(self, ex):
        ...
        memory_bank, layer_wise_outputs = self.encoder(code_rep, \
                                                       ...
                                                       ex["code_word_rep"], \
                                                       ex["adj_matrices_rep"],
                                                       ex['code_td_paths_rep'],
                                                       ex['code_local_relations_rep'])
        # B x (1+seq_len) x h
    ...
    def get_loss(self, logits, ex):
        ...
        if ex["mask_incorrect"].sum() > 0:
            target_loss = (ex["mask_incorrect"].float() * (-torch.log(target_probs + 1e-9))).sum() / (
                        1e-9 + ex["mask_incorrect"].sum())
        else:
...
class VarmisuseModel:
    ...
    # Initialization
    # --------------------------------------------------------------------------
    def __init__(self, args, src_dict, rel_dict=None, type_dict=None, type_dict2=None, state_dict=None,
                 sparse_params={}):
        print(args)
        self.args = args
