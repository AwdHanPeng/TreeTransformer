...
def add_train_args(parser):
    ...
    files.add_argument('--train_src_root_paths', default="paths_train.txt", type=str,
    ...
    # HERE
    files.add_argument('--train_td_src_root_paths', type=str, default="td_paths_train.txt",
                       help='Preprocessed train source root paths file')
    files.add_argument('--dev_td_src_root_paths', type=str, default="td_paths_test.txt",
                       help='Preprocessed dev source root paths file')

    files.add_argument('--train_local_relation', type=str, default="local_relation_train.txt",
                       help='Preprocessed train source root paths file')
    files.add_argument('--dev_local_relation', type=str, default="local_relation_test.txt",
                       help='Preprocessed dev source root paths file')
    ...
    general.add_argument('--use_tqdm', type='bool', default=True,
    ...
    log.add_argument('--comment', type=str, default="", metavar='T',
...
def set_defaults(args):
    ...
    if not args.only_test:
        ...
        # HERE
        if args.use_td_tree_pos_enc:
            train_td_src_root_paths = os.path.join(data_dir, args.train_td_src_root_paths)
            if not os.path.isfile(train_td_src_root_paths):
                raise IOError('No such file: %s' % train_td_src_root_paths)
        else:
            train_td_src_root_paths = None

        # here
        if args.local_relation:
            train_local_relation = os.path.join(data_dir, args.train_local_relation)
            if not os.path.isfile(train_local_relation):
                raise IOError('No such file: %s' % train_local_relation)
        else:
            train_local_relation = None
        ...
        # HERE
        args.train_td_src_root_paths_file = train_td_src_root_paths
        # here
        args.train_local_relation_file = train_local_relation
    ...
    # HERE
    if args.use_td_tree_pos_enc:
        dev_td_src_root_paths = os.path.join(data_dir, args.dev_td_src_root_paths)
        if not os.path.isfile(dev_td_src_root_paths):
            raise IOError('No such file: %s' % dev_td_src_root_paths)
    else:
        dev_td_src_root_paths = None
    # here
    if args.local_relation:
        dev_local_relation = os.path.join(data_dir, args.dev_local_relation)
        if not os.path.isfile(dev_local_relation):
            raise IOError('No such file: %s' % dev_local_relation)
    else:
        dev_local_relation = None
    ...
    # HERE
    args.dev_td_src_root_paths_file = dev_td_src_root_paths
    args.dev_local_relation_file = dev_local_relation
...
def main(args, logger):
    ...
    if args.only_test:
        ...
        model = Code2NaturalLanguage.load(args.model_file, args)
    else:
    ...
    # HERE
    test_files["td_paths"] = args.dev_td_src_root_paths_file
    test_files["local_relation"] = args.dev_local_relation_file
    if not args.only_test:
        train_files = dict()
        ...
        # HERE
        train_files["td_paths"] = args.train_td_src_root_paths_file
        train_files["local_relation"] = args.train_local_relation_file
    ...
    # TRAIN/VALID LOOP
        ...
        for epoch in range(start_epoch, args.num_epochs + 1):
            ...
            # # TODO:TEST for valid! Should delete after checking
